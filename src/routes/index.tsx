    ...
    <div className="h-screen w-full bg-background text-foreground flex flex-col justify-between overflow-hidden relative">
      ...
      <header className="w-full border-b bg-background/50 backdrop-blur-sm sticky top-0 z-50 h-28 flex items-center">
        <div className="flex w-full items-center justify-between px-6 md:px-12 lg:px-20 py-4">
          <div className="flex items-center gap-4 font-serif text-4xl md:text-5xl font-black select-none">
          ...
          <nav className="flex items-center gap-6 text-lg md:text-xl font-bold">
            ...
            <Link to="/auth" className="rounded-full bg-primary hover:bg-[#2c2826] px-6 py-3 text-primary-foreground transition-all duration-300 hover:scale-105 active:scale-95 shadow-sm">Get started</Link>
          </nav>
        </div>
      </header>

      {/* Main hero - centered vertically and horizontally, compact (fits on single screen) */}
      <main className="flex-1 flex flex-col items-center justify-center w-full px-6 md:px-12 lg:px-20 py-4 relative z-10">
        <div className="max-w-4xl flex flex-col items-center text-center">
          <p className="text-xs md:text-sm uppercase tracking-[0.3em] text-muted-foreground/80 font-bold">Vol. 01 — Personal Finance</p>
          <h1 className="mt-4 md:mt-5 font-serif text-4xl sm:text-5xl md:text-6xl lg:text-7xl font-bold tracking-tight leading-[1.08]">
            A warm, quiet place <br />
            <em className="text-accent not-italic">for your money.</em>
          </h1>
          <p className="mt-5 md:mt-6 max-w-2xl text-base md:text-lg text-muted-foreground/90 leading-relaxed mx-auto">
          ...
          <div className="mt-8 md:mt-10 flex flex-wrap justify-center gap-4">
            <Link 
              to="/auth" 
              className="group inline-flex items-center gap-2.5 rounded-full bg-primary hover:bg-[#2c2826] px-8 py-4 text-primary-foreground text-base md:text-lg font-bold transition-all duration-300 hover:scale-105 active:scale-95 shadow-md hover:shadow-lg hover:shadow-accent/15"
            >
              Start tracking 
