...
- **Optimized Page Heights and Spacings (Gap Removal)**:
  ...
  - Adjusted the desktop height constraint of the transaction table parent container (`transactions.tsx`) from `md:h-[calc(100vh-12rem)]` to `md:h-[calc(100vh-8rem)]` to stretch the table and align it beautifully just above the footer.
  - Locked the desktop dashboard container to the exact viewport height bounds (`md:h-[calc(100vh-12rem)] md:overflow-hidden`) using flexible grid and list containers (`flex flex-col md:h-full md:min-h-0`). Charts stretch dynamically to fill parent space and lists (Budget Health, Accounts, Recent Activity) occupy the remaining height (`flex-1`) with inner scrollability. This keeps the entire page perfectly fitting inside a single desktop screen without any page-level scrolling (exactly like the transactions page) while keeping the smaller web footer at the bottom.
...
- **Improved Subscription Payment, Concurrency Locks & Daily Overdue Notifications**:
  ...
  - **Event Transactions (Multi-Record Grouped Records)**:
    ...
    - **Inline Expandable Accordion Event Rows (`transactions.tsx`)**: Removed the popup modal completely. Clicking any Event row or expand toggle (`ChevronDown` / `ChevronRight`) now expands the event inline directly inside the transaction table/list, cleanly displaying all sub-transaction items (icons, categories, accounts, notes, and amounts).
    - **Fixed Page Height & Zero Page Scroller (`transactions.tsx`)**: Sized the Transactions page container to `calc(100vh - 8.5rem)` with `overflow-hidden`. The page fits 100% inside the screen with ZERO page scrollbar, and the inner table scroll container height is neatly constrained above the footer.
    - **Event Editing Capabilities (`transaction-dialog.tsx`)**: Event rows feature a **Pencil (Edit)** button. Clicking it pre-fills the Transaction Dialog in Event Mode with the Event Title, Date, and all sub-records for full editing.
    - **Automated Macro Event Logging (`automation.tsx`)**: Triggering any Macro automation tags generated transactions as a single grouped Event (`🗓️ Event: Macro Name`).
## Live URL
