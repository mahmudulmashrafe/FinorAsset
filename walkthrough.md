...
- **Improved Subscription Payment & Daily Overdue Notifications**:
  ...
  - Set the overdue checks to run every day until there is a sufficient balance in the selected account(s). Once a refilled balance matches the subscription price, the auto-deduction loop immediately processes the payment and advances the due date by 1 month.
