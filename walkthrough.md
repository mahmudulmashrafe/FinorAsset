...
- **Improved Subscription Payment, Concurrency Locks & Daily Overdue Notifications**:
  ...
  - **Event Transactions (Multi-Record Grouped Records)**:
    ...
    - **Full Record Breakdown Modal**: Clicking **View** on any Event row opens a detailed popup showing the complete list of sub-transactions (categories, accounts, amounts, notes), with buttons to edit or delete the event.
    - **Event Editing Capabilities (`transaction-dialog.tsx`)**: Event rows now feature a **Pencil (Edit)** button. Clicking it pre-fills the Transaction Dialog in Event Mode with the Event Title, Date, and all sub-records, allowing full editing of titles, dates, items, categories, accounts, amounts, or notes.
    - **Non-Clickable Event Title & Calendar Icon**: Changed the event icon to a clean calendar badge (`🗓️`) and updated the Event Title text to clean non-clickable typography (preventing accidental clicks).
    - **Automated Macro Event Logging (`automation.tsx`)**: Triggering any Macro automation (e.g., *"Versity Pocket Money"*) automatically tags all generated shortcut transactions as a single grouped Event. In the Transactions history, the macro execution displays as a unified Event item (`🗓️ Event: Versity Pocket Money`) with full record inspection.
## Live URL
