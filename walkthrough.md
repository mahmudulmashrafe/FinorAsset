...
- **Dynamic Category Labels in Macro Cards**: Refactored the macro section in `automation.tsx` to replace the static `"Template shortcuts"` text with a comma-separated list of the actual categories (using icons and names, e.g. `🍔 Food, 🧾 Bills`) associated with the actions in the macro. Also displays `🔄 Transfer` for transfer operations, and fallback values for uncategorized items.
